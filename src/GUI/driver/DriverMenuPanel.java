...
public class DriverMenuPanel extends DriverDashboardPanel { // Assuming DriverDashboardPanel is a JPanel or similar
    ...
    public DriverMenuPanel(Application app, CardLayout cardLayout, JPanel mainPanel, Driver driver) {
        ...
        // or stick to BoxLayout if DriverProfilePanel and OrderInfoPanel are designed for it.
        ...
        for (Component comp : leftPanel.getComponents()) {
            if (comp instanceof JButton) {
                ...
                if ("Profile".equals(button.getText())) {
                    // Ensure showProfilePanel itself doesn't rely on nowPressed being set before this
                    // If showProfilePanel calls setButtonPressed, this might be redundant or need coordination
    ...
    private JPanel buildLeftPanel() {
        ...
        welcomeLabel.setMaximumSize(new Dimension(Integer.MAX_VALUE, 57)); // Span full width
        welcomeLabel.setPreferredSize(new Dimension(389,57));
    ...
    // Sets the visual state of a newly pressed button and resets the previously pressed one
    private void setButtonPressed(JButton currentButton, JButton previousButton) {
        ...
        // Reset style for the previously pressed button (if one exists and is not the current one)
        ...
        // It's generally better to repaint the specific components or their immediate parent
        if (currentButton != null) currentButton.repaint();
        if (previousButton != null) previousButton.repaint();
        // SwingUtilities.updateComponentTreeUI can be heavy; use repaint for simple color changes.
    }
    ...
    private void showOrderInfoPanel() {
        ...
        // Assuming getOrderInfoPanel() returns a JPanel, likely a CustomerOrderDetailPanel instance
        // It's crucial that this panel uses null layout if we are setting bounds manually for new buttons
        JPanel orderInfoPanel = driver.getOrder().getOrderInfoPanel();
        if (orderInfoPanel.getLayout() != null) {
             // If it's not null layout, adding components with setBounds won't work as expected.
        ...
        // This could also be a separate private method like styleDriverActionButton(JButton button)
        ...
        // buttonPanel.setOpaque(false); // If orderInfoPanel background should show through
        ...
        chatButton.addActionListener(e -> {
            if (driver.getOrder() != null) driver.getOrder().showChat(driver);
        });
        // chatButton.setBounds(100, buttonY, 150, buttonHeight); // Manual bounds if orderInfoPanel is null layout
        ...
        dropOffButton.addActionListener(e -> {
            if (driver.getOrder() != null) {
                ...
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    // DriverMenuPanel newMenu = new DriverMenuPanel(this.app, this.cardLayout, this.mainPanel, this.driver);
        ...
        // If orderInfoPanel uses null layout, you'd set bounds for buttonPanel or add buttons directly.
        // If orderInfoPanel uses a layout manager (e.g., BorderLayout), add buttonPanel to a region.
        // Assuming orderInfoPanel might not be null layout, or we want more control:
        // We can add the buttonPanel to the SOUTH of the orderInfoPanel if it's BorderLayout.
        // Or, if orderInfoPanel is complex, we might need to ensure it has space.

        // For now, if orderInfoPanel is null layout, we need to set bounds for buttonPanel
        // buttonPanel.setBounds(0, buttonY, orderInfoPanel.getWidth(), buttonHeight + 20); // Example bounds for buttonPanel
        ...
        // Reverting to direct add with setBounds if that's the expectation for orderInfoPanel:
        // Remove buttonPanel creation and adding, uncomment setBounds for individual buttons.
        // For this example, I'll assume orderInfoPanel can have a button panel added to its south or uses null layout.
        // Let's stick to the user's original intent of adding buttons directly with setBounds,
    ...
    private void logout() {
        int confirm = JOptionPane.showConfirmDialog(this, "Are you sure you want to logout?", "Logout Confirmation",
                JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
        if (confirm == JOptionPane.YES_OPTION) {
            // Perform any actual logout logic if needed (e.g., clearing session, notifying app)
    ...
    // static class DriverProfilePanel extends JPanel { public DriverProfilePanel(Driver d){ setBackground(Color.CYAN); add(new JLabel("Profile of " + d.getName()));} }
